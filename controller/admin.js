    ...
    try {
      if (req.user.role === "admin") {
        ...
        if (!findUser) {
          return res
            .status(404)
            .json("User Not Found, make sure the user is registered");
        }
        if (findUser.role === "admin") {
          return res
            .status(406)
            .json("Not Acceptable, this user is already an admin");
        }
        ...
        res
          .status(201)
          .json(`${toBeAdmin} is now an admin by ${req.user.name}`);
      } else {
        if (req.user.role !== "admin") {
    ...
    try {
      ...
      if (!findFood) {
        return res
          .status(404)
          .json(
            `The food with the name ${foodToMakeAvaiableOrNot} is not a food here yet, do well to add the food first`
          );
      }
      ...
      if (findFood.available) {
        ...
        return res
          .status(201)
          .json(`${findFood.name} is no more available for order`);
      }
      ...
      if (!findFood.available) {
        ...
        return res
          .status(201)
          .json(`${findFood.name} is now available for ordering`);
      }
    } catch (error) {
      next(error);
    }
